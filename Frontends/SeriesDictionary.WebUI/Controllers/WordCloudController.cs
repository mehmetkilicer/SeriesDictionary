...
namespace SeriesDictionary.WebUI.Controllers
{
    public class WordCloudController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7257/api/Show/wordcloud");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<GetWordCloudQueryResult>>(jsonData);
                return View(values);
            }
            return View();
        }
